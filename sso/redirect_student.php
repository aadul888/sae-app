...
if (!empty($_COOKIE['ADMIN_KEY']) && !empty($_COOKIE['KEY'])) {
    ...
    if ($admin_id > 0) {
        $qa = "SELECT admin_id, username, email, active FROM admin WHERE admin_id='" . $admin_id . "' LIMIT 1";
        $ra = $connection->query($qa);
        if ($ra && $ra->num_rows > 0) {
...
if ($payload === null && !empty($_COOKIE['siswa'])) {
    ...
    if ($user_id > 0) {
        $qu = "SELECT u.user_id, u.nisn, u.status, u.tingkat, u.kelas_nama, k.nama_kelas, k.tingkat_pendidikan_id
               FROM user u
               LEFT JOIN kelas k ON u.kelas = k.kelas_id
               WHERE u.user_id='" . $user_id . "' LIMIT 1";
        $ru = $connection->query($qu);
        if ($ru && $ru->num_rows > 0) {
            $user = $ru->fetch_assoc();
