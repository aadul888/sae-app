 ...
 *    - Payload URL: https://domain-anda.com/webhook.php
...
// Priority 1: git fast-forward merge
if ($git_dir && file_exists($git_bin)) {
    ...
    if ($return_var === 0) {
        $cmd_merge = "$git_safe merge --ff-only origin/main 2>&1";
        exec($cmd_merge, $output, $return_var);
        if ($return_var === 0) {
            $deployed = true;
        } else {
            // fallback pull
            $cmd_pull = "$git_safe pull --ff-only 2>&1";
            exec($cmd_pull, $output, $return_var);
