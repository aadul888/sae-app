...
// --- Routing guard: harus SEBELUM kode yang butuh koneksi DB ---
...
// Registrasi bootstrap di-skip — modul registrasi tidak aktif
$bootstrap_required = false;
$bootstrap_completed = false;

$request_path = parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH) ?: '/';

// ===== GATE: Setup routing based on database state ONLY =====
// Jika belum setup (credentials kosong), redirect ke /installer
if (!sae_setup_complete()) {
  if (!preg_match('#/installer(?:\.php)?/?$#i', $request_path)) {
...
$website_name       = $row_site['site_name'] ?? 'Smart Apps Education';
$website_phone      = $row_site['site_phone'] ?? '';
$website_addres     = $row_site['site_address'] ?? '';
$website_logo       = $row_site['site_logo'] ?? 'logoweb1.png';
