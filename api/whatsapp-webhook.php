...
try {
    ...
    // Autentikasi: verifikasi shared secret (api_key dari whatsapp_config)
    // Pengirim (gateway) harus mengirim header X-API-Key atau Authorization: Bearer
    ...
    $cfg_result = $connection->query("SELECT api_key FROM whatsapp_config WHERE id = 1 LIMIT 1");
    $expected_key = ($cfg_result && $cfg_result->num_rows > 0) ? (string) $cfg_result->fetch_row()[0] : '';
