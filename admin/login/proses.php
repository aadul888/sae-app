...
if (!function_exists('ensure_default_admin_account')) {
  ...
  {
    $default_username = trim((string) getenv('DEFAULT_ADMIN_USERNAME'));
    $default_password = trim((string) getenv('DEFAULT_ADMIN_PASSWORD'));
...
if (!hash_equals($_SESSION['csrf_token'] ?? '', $csrf_token)) {
  // Optionally validate, but login is exempted from strict CSRF to allow bookmark access
  // Actual CSRF applied to write operations in each module
}

switch (@$_GET['action']){
case 'login':
  // Rate limiting: max 10 attempts per IP per 5 minutes
  if (!rate_limit_check($connection, 'login', 10, 300)) {
  ...
  if (empty($_POST['username'])) { 
      ...
      $username = $_POST['username']; // will use prepared statement
  }
...
if (empty($error)){
  ...
  $stmt = $connection->prepare("SELECT * FROM admin WHERE username=? AND active='Y' LIMIT 1");
  if (!$stmt) {
  ...
  if($result_login->num_rows > 0){
    ...
    $stmt->close();
    if(sync_pengguna_password_matches($password_plain, $admin_row['password'])) {
      // Regenerate session to prevent session fixation
      session_regenerate_id(true);

      $ADMIN_KEY          = htmlentities(epm_encode($admin_row['admin_id']));
      $KEY                = hash('sha256',$admin_row['username']);
      // Create session-bound cookie signatures to prevent cookie theft reuse
      $cookie_bind = cookie_bind_session($ADMIN_KEY . '|' . $KEY);

      /* ---------- Update Status Online --------- */
      $update_stmt = $connection->prepare("UPDATE admin SET time=?, status='Online' WHERE admin_id=?");
      if ($update_stmt) {
        $update_stmt->bind_param('ii', $time_online, $admin_row['admin_id']);
        $update_stmt->execute();
        $update_stmt->close();
      }
      /* ---------- Update Status Online --------- */
      secure_setcookie('ADMIN_KEY', $ADMIN_KEY, $expired_cookie, '/');
      secure_setcookie('KEY', $KEY, $expired_cookie, '/');
      secure_setcookie('level_id', $admin_row['level_id'], $expired_cookie, '/');
      secure_setcookie('cookie_bind', $cookie_bind, $expired_cookie, '/');
      if (!empty($admin_row['tugas_tambahan'])) {
        secure_setcookie('tugas_tambahan', $admin_row['tugas_tambahan'], $expired_cookie, '/');
      }
      echo'success';
    }else{
