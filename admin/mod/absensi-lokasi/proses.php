...
switch ($aksi) {
    ...
    case 'simpan':
        ...
        if (empty($error)) {
            if ($id == '') {
                $st = $connection->prepare("INSERT INTO lokasi (nama_lokasi, keterangan, latitude, longitude, radius, status) VALUES (?, ?, ?, ?, ?, ?)");
                $st->bind_param('ssddds', $nama, $ket, $lat, $lng, $rad, $stat);
                $exec = $st->execute();
                $st->close();
            } else {
                $st = $connection->prepare("UPDATE lokasi SET nama_lokasi=?, keterangan=?, latitude=?, longitude=?, radius=?, status=? WHERE lokasi_id=?");
                $st->bind_param('ssdddss', $nama, $ket, $lat, $lng, $rad, $stat, $id);
                $exec = $st->execute();
                $st->close();
            }
    ...
    case 'delete':
        ...
        header('Content-Type: application/json');
        $id = (int)$_POST['lokasi_id'];
        $stmt_dl = $connection->prepare("DELETE FROM lokasi WHERE lokasi_id=?");
        $stmt_dl->bind_param('i', $id);
        if ($stmt_dl->execute()) {
            echo json_encode(['success' => true]);
        } else {
