...
function showBootstrapModal(d, avatarSrc, kkPreview, kksPreview, kipPreview, id) {
...
function showFallbackModal(d, avatarSrc, kkPreview, kksPreview, kipPreview, id) {
  createFallbackModal();
...
if (!$('link[href*="usulan-pip-rangking/style.css"]').length) {
  $('<link rel="stylesheet" type="text/css" href="./mod/usulan-pip-ranking/style.css">').appendTo('head');
}
  ...
  $(document).on('click', '.img-kk-thumb', function(e) {
    ...
    $('#pipPreviewModal .modal-title').html('<i class="fas fa-eye mr-2"></i>Preview Berkas KK');
...
window.resetImage = function() {
  ...
  $(document).on('click', '.img-kks-thumb', function(e) {
    ...
    $('#pipPreviewModal .modal-title').html('<i class="fas fa-eye mr-2"></i>Preview Berkas KKS');
    $('#pipPreviewModalBody').html(html);
    $('#pipPreviewModal').modal('show');
  });
  ...
  $(document).on('click', '.img-kip-thumb', function(e) {
    ...
    $('#pipPreviewModal .modal-title').html('<i class="fas fa-eye mr-2"></i>Preview Berkas KIP');
    $('#pipPreviewModalBody').html(html);
    $('#pipPreviewModal').modal('show');
  });

  // Prevent bubbling for thumbnail images so modal works
  $(document).on('click', '.img-kk-thumb, .img-kks-thumb, .img-kip-thumb', function(e) {
    e.stopPropagation();
  });
  ...
  $(document).on('click', '.btn-view, .table-action-view, .btn-stts, .btn-delete, .btn-approve, .btn-reject, .btn-submit-reject, .btn-submit-approve, .btn-cancel-approve, .btn-cancel-reject, .modal-close-btn, .file-download-link', function(e) {
    ...
    if ($self.hasClass('btn-approve')) {
      ...
      swal({
        ...
        icon: 'warning',
        buttons: { cancel: true, confirm: { text: 'Setujui', closeModal: false } }
      }).then(function (confirmed) {
        if (!confirmed) return;
    $.post('./mod/usulan-pip-ranking/proses.php?action=status', { id: id, status: 'Y' }, function (data) {
          var resp = data;
        ...
        }).fail(function (xhr) { swal('Error!', 'Request gagal: ' + (xhr.statusText || xhr.status), 'error'); });
      });
      return;
    }
    ...
    if ($self.hasClass('btn-submit-reject')) {
      ...
      $.post('./mod/usulan-pip-ranking/proses.php?action=status', { 
        id: id, 
        status: 'N',
        reason: reason
      }, function (data) {
  ...
  $.post('./mod/usulan-pip-ranking/proses.php?action=status', { id: id, status: status }, function (data) {
        if (data == 'success') {
    ...
    if ($self.hasClass('btn-delete')) {
      ...
      swal({ text: 'Anda yakin ingin menghapus data ini?', icon: 'warning', buttons: { cancel: true, confirm: true }, value: 'yes' })
        .then(function (value) {
          ...
          loading();
          $.post('./mod/usulan-pip-ranking/proses.php?action=delete', { id: id }, function (data) {
            var resp = data;
            try { if (typeof data === 'string') resp = JSON.parse(data); } catch (e) { /* ignore */ }
            if (resp && resp.success) {
...
function fetchAndShowModal(id) {
  ...
  url: './mod/usulan-pip-ranking/proses.php',
    ...
    if (typeof $.fn.modal !== 'undefined') {
      showBootstrapModal(d, avatarSrc, kkPreview, kksPreview, kipPreview, id);
    } else {
      console.warn('[usulan-pip] Bootstrap modal not available, using fallback');
      showFallbackModal(d, avatarSrc, kkPreview, kksPreview, kipPreview, id);
    }
  }).fail(function (xhr) {
...
$(document).on('click', '.btn-set-pos', function (e) {
  ...
  swal({
    ...
    if (isNaN(posisi) || posisi < 1) {
      swal('Error!', 'Posisi tidak valid. Masukkan angka minimal 1.', 'error');
      return;
    }
    ...
    $.ajax({
      ...
      if (resp && resp.success) {
        swal('Berhasil!', 'Posisi ranking berhasil diubah ke #' + posisi, 'success');
        if (table && typeof table.ajax !== 'undefined') table.ajax.reload(null, false);
      } else {
        var errorMsg = resp && resp.message ? resp.message : 'Terjadi kesalahan';
      ...
      if (xhr.status === 403) {
        errorMsg = 'Akses ditolak. Hanya wali kelas yang dapat mengatur ranking.';
...
$(document).on('click', '.btn-move-up, .btn-move-down', function (e) {
  ...
  $.ajax({
    ...
    if (resp && resp.success) {
      swal('Berhasil!', 'Posisi ranking berhasil dipindah ke ' + dirText, 'success');
      if (table && typeof table.ajax !== 'undefined') table.ajax.reload(null, false);
    } else {
      var errorMsg = resp && resp.message ? resp.message : 'Terjadi kesalahan';
...
var savedKelas = localStorage.getItem('usulan_pip_ranking_filter_kelas') || '';
var savedStatus = localStorage.getItem('usulan_pip_ranking_filter_status') || '';
var savedDapodik = localStorage.getItem('usulan_pip_ranking_filter_dapodik') || '';
...
$(document).on('change', '.dapodik-checkbox', function() {
  ...
  var statusText = newStatus === 'Y' ? 'sudah input ke Dapodik' : 'belum input ke Dapodik';
  ...
  $.ajax({
          ...
          table.ajax.reload(function(json) {
            ...
            setTimeout(function() {
              $('.dapodik-checkbox').each(function() {
                var $cb = $(this);
                var id = $cb.data('id');
                console.log('Syncing checkbox for ID:', id, 'Current checked:', $cb.is(':checked'));
    ...
    error: function(xhr) {
      ...
      if (xhr.status === 403) {
        errorMsg = 'Akses ditolak. Hanya superadmin dan admin yang dapat mengubah status dapodik.';
      }
