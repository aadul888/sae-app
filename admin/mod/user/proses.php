...
require_once '../../../library/qrcode_helper.php';
...
if (isset($_POST['op'])) {
  ...
  switch ($op) {
    case 'regenerate_qrcode':
      if ($data_role['modifikasi'] == 'Y' && isset($_POST['user_id'])) {
        $user_id = intval(epm_decode($_POST['user_id']));
        $stmt_u = $connection->prepare("SELECT nisn FROM user WHERE user_id=?");
        $stmt_u->bind_param('i', $user_id);
        $stmt_u->execute();
        $query_user = $stmt_u->get_result();
        if ($query_user && $query_user->num_rows > 0) {
          ...
          if (!empty($nisn)) {
            ...
            ensure_qrcode_directory_exists();

            // Generate the new QR code
            $new_qrcode_path = generate_user_qrcode($nisn, $koneksi_server);

            if (!empty($new_qrcode_path)) {
              // Prepend cache-busting timestamp
              $response = array(
                'success' => true,
                'message' => 'QR Code berhasil dibuat ulang.',
                'qrcode_url' => $new_qrcode_path . '?t=' . time()
              );
            } else {
              $response = array('success' => false, 'message' => 'Gagal membuat QR Code.');
            }
          } else {
            $response = array('success' => false, 'message' => 'NISN tidak ditemukan untuk user ini.');
          }
        } else {
          $response = array('success' => false, 'message' => 'User tidak ditemukan.');
        }
      } else {
        $response = array('success' => false, 'message' => 'Akses ditolak atau ID User tidak valid.');
      }
      echo json_encode($response);
      break;
    default:
      // Fallback for original switch
      if ($data_role['modifikasi'] == 'Y') {
        // ... (existing cases like 'update', 'delete', etc.)
      }
      break;
  }
...
switch (@$_GET['action']) {
  ...
  case 'set_koordinator':
    ...
    $stmt = $connection->prepare("UPDATE user SET koordinator=? WHERE user_id=?");
    $stmt->bind_param('is', $set, $id);
    $update = $stmt->execute();
    $stmt->close();
    if ($update) {
      echo 'success';
  ...
  case 'search':
    ...
    if ($query_raw === '') {
      // No value selected â€” return empty result. The client will handle showing/clearing results.
      break;
    }
  ...
  case 'search_options':
    ...
    if ($search_by === 'email') {
      ...
      // nik and no_kk combined â€” only duplicates
  ...
  case 'delete':
    ...
    $id = epm_decode($_POST['id']);
    // Hapus avatar
    $stmt_user = $connection->prepare("SELECT avatar, nisn FROM user WHERE user_id=?");
    ...
    } else { $stmt_user->close(); }
    // Hapus semua berkas di berkas_siswa
    $stmt_berkas = $connection->prepare("SELECT kk, ijazah, akte, kip, kks FROM berkas_siswa WHERE user_id=?");
    $stmt_berkas->bind_param('s', $id);
    $stmt_berkas->execute();
    $q_berkas = $stmt_berkas->get_result();
    ...
    $stmt_del1 = $connection->prepare("DELETE FROM berkas_siswa WHERE user_id=?");
    $stmt_del1->bind_param('s', $id);
    $stmt_del1->execute();
    $stmt_del1->close();
    $stmt_del2 = $connection->prepare("DELETE FROM statistik WHERE user_id=?");
    $stmt_del2->bind_param('s', $id);
    $stmt_del2->execute();
    $stmt_del2->close();
    $stmt_del3 = $connection->prepare("DELETE FROM usulan WHERE user_id=?");
    $stmt_del3->bind_param('s', $id);
    $stmt_del3->execute();
    $stmt_del3->close();
    // Jika ada tabel yang pakai nisn
    if (!empty($nisn)) {
      $stmt_del4 = $connection->prepare("DELETE FROM berkas_siswa WHERE nisn=?");
      $stmt_del4->bind_param('s', $nisn);
      $stmt_del4->execute();
      $stmt_del4->close();
      $stmt_del5 = $connection->prepare("DELETE FROM statistik WHERE nisn=?");
      $stmt_del5->bind_param('s', $nisn);
      $stmt_del5->execute();
      $stmt_del5->close();
      $stmt_del6 = $connection->prepare("DELETE FROM usulan WHERE nisn=?");
      $stmt_del6->bind_param('s', $nisn);
      $stmt_del6->execute();
      $stmt_del6->close();
    }
    // Hapus data user
    $stmt_del_user = $connection->prepare("DELETE FROM user WHERE user_id=?");
    $stmt_del_user->bind_param('s', $id);
    ...
    if (isset($_FILES['photo_files']) && $_FILES['photo_files']['error'] == 0) {
      ...
      if ($file_ext == 'zip') {
        ...
        if ($zip->open($file_tmp) === TRUE) {
          for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (in_array($zip_ext, ['jpg', 'jpeg', 'png'])) {
              ...
              $query = $connection->query("SELECT user_id, avatar FROM user WHERE nisn='" . $nisn_esc . "'");
              $user_id_for_cache = '';
              ...
              if ($stream) {
                ...
                $stmt_zu->bind_param('ss', $avatar_db, $nisn);
        ...
        if (move_uploaded_file($file_tmp, $target_dir . $new_name)) {
          ...
          $stmt_upd_avatar = $connection->prepare("UPDATE user SET avatar=? WHERE nisn=?");
          $stmt_upd_avatar->bind_param('ss', $avatar_db, $nisn);
          $update = $stmt_upd_avatar->execute();
          $stmt_upd_avatar->close();
          if ($update) {
            purgeKartuPelajarCacheByIdentity($user_id_for_cache, $nisn);
  ...
  case 'reset_password_simple':
    check_access('modifikasi');
    $id = epm_decode($_POST['id']);
    $stmt = $connection->prepare("SELECT nisn, nama_lengkap FROM user WHERE user_id=?");
    ...
    if ($result && $result->num_rows > 0) {
      ...
      $stmt_upd = $connection->prepare("UPDATE user SET password=? WHERE user_id=?");
      $stmt_upd->bind_param('ss', $password_hash, $id);
  ...
  case 'reset_password':
    check_access('modifikasi');
    $id = epm_decode($_POST['id']);
    $stmt = $connection->prepare("SELECT nisn, nama_lengkap, telp, whatsapp_verified FROM user WHERE user_id=?");
    $stmt->bind_param('s', $id);
    $stmt->execute();
    $result = $stmt->get_result();
    $stmt->close();
    if ($result && $result->num_rows > 0) {
      ...
      if ($update) {
        ...
        if (!empty($telp) && $wa_verified == 1) {
          ...
          // Pesan WhatsApp
          $wa_message = "ðŸ”’ *RESET PASSWORD SAE SMK NEGERI 1 PAGELARAN*\n\n";
          $wa_message .= "Halo *{$nama}*,\n\n";
          $wa_message .= "Admin telah mereset password akun Anda!\n\n";
          $wa_message .= "ðŸ“‹ *Detail Akun:*\n";
          $wa_message .= "â€¢ Username/NISN: *{$nisn}*\n";
          $wa_message .= "â€¢ Password: *{$nisn}*\n";
          $wa_message .= "â€¢ Status: Aktif\n\n";
          $wa_message .= "ðŸŒ *Link Login:*\n";
          $wa_message .= "https://sae.smakpal.sch.id/login\n\n";
          $wa_message .= "âš ï¸ *PENTING:*\n";
          $wa_message .= "â€¢ Segera login dan ubah password Anda\n";
          $wa_message .= "â€¢ Jangan berikan informasi ini kepada siapa pun\n\n";
  ...
  case 'reset_konfirmasi':
    ...
    $stmt = $connection->prepare("UPDATE user SET konfirmasi='Belum Konfirmasi' WHERE user_id=?");
    $stmt->bind_param('s', $id);
    $update = $stmt->execute();
    $stmt->close();
    if ($update) {
  ...
  case 'delete_all':
    ...
    if ($result && $result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        ...
        $stmt_du->bind_param('s', $id); $stmt_du->execute(); $stmt_du->close();
        purgeKartuPelajarCacheByIdentity($id, $nisn);
        $deleted_count++;
      }
      echo 'success';
  ...
  case 'upload_foto_nisn':
    ...
    if (isset($_FILES['foto']) && isset($_POST['nisn'])) {
      ...
      // Hapus foto lama jika ada (selain avatar.jpg)
      $stmt_av = $connection->prepare("SELECT user_id, avatar FROM user WHERE nisn=?");
      $stmt_av->bind_param('s', $nisn);
      $stmt_av->execute();
      $query = $stmt_av->get_result();
      $user_id_for_cache = '';
      if ($query && $query->num_rows > 0) {
        $row = $query->fetch_assoc();
        $stmt_av->close();
        $user_id_for_cache = $row['user_id'];
        $old_avatar = $row['avatar'];
        $old_avatar_file = preg_replace('/\?.*/', '', $old_avatar);
        $old_path = $target_dir . $old_avatar_file;
        if (file_exists($old_path) && $old_avatar_file !== 'avatar.jpg') {
      ...
      $stmt_upd_av = $connection->prepare("UPDATE user SET avatar=? WHERE nisn=?");
      $stmt_upd_av->bind_param('ss', $avatar_db, $nisn);
      $update = $stmt_upd_av->execute();
      $stmt_upd_av->close();
      if ($update) {
        purgeKartuPelajarCacheByIdentity($user_id_for_cache, $nisn);
        echo 'success';
