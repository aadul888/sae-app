...
$fs_root = realpath(__DIR__ . '/../../..') . DIRECTORY_SEPARATOR; // points to app root
$http = isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) !== 'off' ? 'https' : 'http';
...
$possible_logo_fs = [
    $fs_root . 'content/assets/logo-jurusan/' . $logo_jurusan_file,
    $fs_root . 'content/logo-jurusan/' . $logo_jurusan_file,
    $fs_root . 'content/' . $logo_jurusan_file
];
foreach ($possible_logo_fs as $chk_fs) {
    if (file_exists($chk_fs)) {
        $paths_fs['logo_jurusan'] = $chk_fs;
        $rel = str_replace($fs_root, '', $chk_fs);
        $paths_web['logo_jurusan'] = $web_root . '/' . str_replace(DIRECTORY_SEPARATOR, '/', $rel) . '?t=' . filemtime($chk_fs);
        break;
    }
}
