    ...
    case 'proses_sanggah':
        ...
        if ($stmt->execute()) {
            ...
            if ($status === 'Disetujui' && $poin_kurang > 0) {
                ...
                // Get current poin
                $qp = $connection->query("SELECT poin_diberikan FROM poin_pelanggaran WHERE pelanggaran_id=$pel_id LIMIT 1");
                if ($qp) {
                    $cur_poin = intval($qp->fetch_assoc()['poin_diberikan']);
                    $new_poin = max(0, $cur_poin - $poin_kurang);
                    $connection->query("UPDATE poin_pelanggaran SET poin_diberikan=$new_poin, status='Dikurangi' WHERE pelanggaran_id=$pel_id");
                }
            } elseif ($status === 'Disetujui' && $poin_kurang == 0) {
                // Full removal
                $connection->query("UPDATE poin_pelanggaran SET status='Disanggah' WHERE pelanggaran_id=" . intval($sanggah['pelanggaran_id']));
            }
