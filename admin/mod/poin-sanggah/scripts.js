...
$(document).ready(function () {
  var prosesUrl = "./mod/poin-sanggah/proses.php";
  var dtUrl = "./mod/poin-sanggah/datatable.php";

  var table = $("#tbl-sanggah").DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      url: dtUrl,
      type: "POST",
      data: function (d) {
        d.status = $("#filter-status-sanggah").val();
      }
    },
    columns: [
    ...
    language: {
      ...
      zeroRecords: "Tidak ada sanggahan",
  ...
  $("#filter-status-sanggah").on("change", function () { table.ajax.reload(); });

  // Proses sanggah
  $(document).on("click", ".btn-proses-sanggah", function () {
    var id = $(this).data("id");
    $("#sanggah-id-proses").val(id);
    $("#sanggah-detail-content").html('<div class="text-center py-3"><i class="fas fa-spinner fa-spin fa-2x"></i></div>');
    $("#form-proses-sanggah")[0].reset();
    $("#sanggah-keputusan").val("Disetujui");
    $("#sanggah-setuju-fields").show();
    $("#modal-proses-sanggah").modal("show");

    $.post(prosesUrl, { action: "detail_sanggah", sanggah_id: id }, function (res) {
      if (res.status === "success") {
        var d = res.data;
        var html = '<div class="row">';
        html += '<div class="col-md-6"><p class="mb-1"><strong>Siswa:</strong> ' + d.nama_lengkap + ' (' + d.nisn + ')</p>';
        html += '<p class="mb-1"><strong>Kelas:</strong> ' + (d.nama_kelas || '-') + '</p>';
        html += '<p class="mb-1"><strong>Total Poin Aktif:</strong> <span class="badge badge-' + (parseInt(d.total_poin) >= 100 ? 'danger' : (parseInt(d.total_poin) >= 70 ? 'warning' : 'primary')) + '">' + d.total_poin + '</span></p></div>';
        html += '<div class="col-md-6"><p class="mb-1"><strong>Pelanggaran:</strong> ' + d.jenis_pelanggaran + '</p>';
        html += '<p class="mb-1"><strong>Poin:</strong> <span class="badge badge-danger">' + d.poin_diberikan + '</span></p>';
        html += '<p class="mb-1"><strong>Tgl Kejadian:</strong> ' + d.tanggal_kejadian + '</p></div></div>';
        html += '<div class="alert alert-light mt-2"><strong>Alasan Sanggahan:</strong><br>' + (d.alasan || '-') + '</div>';
        if (d.bukti_sanggah) {
          html += '<p><strong>Bukti:</strong> <a href="../content/berkas/sanggah/' + d.bukti_sanggah + '" target="_blank">Lihat Bukti</a></p>';
        }
        html += '<p class="mb-0"><strong>Jenis:</strong> <span class="badge badge-info">' + d.jenis_sanggah + '</span> | <strong>Diajukan:</strong> ' + d.tanggal_pengajuan + '</p>';
        $("#sanggah-detail-content").html(html);
        $("#sanggah-poin-kurang").attr("max", d.poin_diberikan).val(d.poin_diberikan);
      } else {
        $("#sanggah-detail-content").html('<p class="text-danger">' + res.message + '</p>');
      }
    }, "json");
  });

  $("#sanggah-keputusan").on("change", function () {
    if ($(this).val() === "Disetujui") {
      $("#sanggah-setuju-fields").slideDown();
    } else {
      $("#sanggah-setuju-fields").slideUp();
    }
  });

  $("#form-proses-sanggah").on("submit", function (e) {
    e.preventDefault();
    ...
    $.ajax({
      url: prosesUrl,
      type: "POST",
      data: $(this).serialize() + "&action=proses_sanggah",
      dataType: "json",
      success: function (res) {
        if (res.status === "success") {
          swal({ title: "Berhasil!", text: res.message, icon: "success", timer: 1500 }).then(function () {
            $("#modal-proses-sanggah").modal("hide");
            table.ajax.reload();
          });
        } else {
          swal("Gagal", res.message, "error");
          btn.prop("disabled", false).html('<i class="fas fa-save mr-1"></i>Simpan Keputusan');
        }
      },
      error: function () {
  ...
  // Selesai sanggah
  $(document).on("click", ".btn-selesai-sanggah", function () {
    var id = $(this).data("id");
    swal({
      title: "Tandai Selesai?",
      text: "Siswa telah memenuhi syarat/kesepakatan sanggahan",
      icon: "info",
      buttons: { cancel: "Batal", confirm: "Ya, Selesai!" }
    }).then(function (ok) {
      if (ok) {
        $.post(prosesUrl, { action: "selesai_sanggah", sanggah_id: id }, function (res) {
          if (res.status === "success") {
