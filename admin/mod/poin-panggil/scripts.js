...
$(document).ready(function () {
  var prosesUrl = "./mod/poin-panggil/proses.php";
  var dtUrl = "./mod/poin-panggil/datatable.php";

  var table = $("#tbl-panggil").DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      url: dtUrl,
      type: "POST",
      data: function (d) { d.status = $("#filter-status-panggil").val(); }
    },
    columns: [
    ...
    language: {
      ...
      zeroRecords: "Tidak ada data pemanggilan",
  ...
  $("#filter-status-panggil").on("change", function () { table.ajax.reload(); });

  var today = new Date().toISOString().split("T")[0];

  // Buat panggilan
  $(document).on("click", ".btn-buat-panggilan", function () {
    $("#form-panggilan")[0].reset();
    $("#panggil-userid").val($(this).data("userid"));
    $("#panggil-total-poin").val($(this).data("poin"));
    $("#panggil-nama").text($(this).data("nama"));
    $("#panggil-poin-badge").text($(this).data("poin"));
    $("input[name='tanggal_panggil']").val(today);
    $("#modal-panggilan").modal("show");
  });

  $("#form-panggilan").on("submit", function (e) {
    e.preventDefault();
    ...
    $.ajax({
      url: prosesUrl,
      type: "POST",
      data: $(this).serialize() + "&action=buat_panggilan",
      dataType: "json",
      success: function (res) {
        if (res.status === "success") {
          swal({ title: "Berhasil!", text: res.message, icon: "success", timer: 1500 }).then(function () { location.reload(); });
        } else {
          swal("Gagal", res.message, "error");
          btn.prop("disabled", false).html('<i class="fas fa-phone mr-1"></i>Buat Panggilan');
        }
      },
      error: function () {
  ...
  // Isi hasil
  $(document).on("click", ".btn-hasil", function () {
    $("#form-hasil")[0].reset();
    $("#hasil-panggil-id").val($(this).data("id"));
    $("input[name='tanggal_hadir']").val(today);
    $("#modal-hasil-panggilan").modal("show");
  });
  ...
  // Selesai
  $(document).on("click", ".btn-selesai-panggil", function () {
    var id = $(this).data("id");
    swal({
      title: "Tandai Selesai?",
      text: "Proses pemanggilan selesai",
      icon: "info",
      buttons: { cancel: "Batal", confirm: "Ya, Selesai!" }
    }).then(function (ok) {
      if (ok) {
        $.post(prosesUrl, { action: "selesai_panggilan", panggil_id: id }, function (res) {
          if (res.status === "success") {
            swal({ title: "Berhasil!", text: res.message, icon: "success", timer: 1500 }).then(function () { table.ajax.reload(); location.reload(); });
