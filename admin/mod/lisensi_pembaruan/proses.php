...
switch (@$_GET['action']) {
  ...
  case 'deploy':
    ...
    // Priority 1: git fast-forward merge (tidak hapus file untracked)
    if ($git_dir && file_exists($git_bin)) {
      chdir($git_dir);
      putenv('GIT_TERMINAL_PROMPT=0');
      exec("$git_safe fetch origin 2>&1", $output, $return_var);

      $local_hash = trim(shell_exec("$git_safe rev-parse HEAD"));
      $remote_hash = trim(shell_exec("$git_safe rev-parse origin/main"));

      if ($local_hash === $remote_hash) {
        echo json_encode(['success' => true, 'message' => 'Aplikasi sudah versi terbaru.']);
        exit;
      }

      if ($return_var === 0) {
        exec("$git_safe merge --ff-only origin/main 2>&1", $output, $return_var);
        if ($return_var === 0) {
          $deployed = true;
        } else {
          // fallback: pull biasa
          exec("$git_safe pull --ff-only 2>&1", $output, $return_var);
          if ($return_var === 0) $deployed = true;
          else $err = 'git merge gagal: ' . implode("\n", $output);
        }
      } else {
    ...
    if ($deployed) {
      ...
      if ($mig_result['ran'] > 0) {
        $connection->query("INSERT INTO pembaharuan (version, pembaharuan, release_date, created_at) 
                            VALUES ('db-mig', 'Migrasi database', NOW(), NOW())");
      }
  ...
  case 'check_remote_commits':
    ...
    if ($git_dir && file_exists($git_bin)) {
      chdir($git_dir);
      putenv('GIT_TERMINAL_PROMPT=0');
      exec("$git_safe fetch origin 2>&1");
