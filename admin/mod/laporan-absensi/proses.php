...
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])) {
  ...
  switch (@$_GET['action']) {
    ...
    case 'edit-form':
      ...
      $id = $_POST['id'];
      $query = "SELECT a.*, u.nama_lengkap, u.nisn FROM absensi a LEFT JOIN user u ON a.user_id = u.user_id WHERE a.id='$id'";
    ...
    case 'update':
      ...
      // Update data ke database, handle value kosong agar tetap NULL
      $jam_masuk_sql = ($jam_masuk === '' || $jam_masuk === null) ? 'NULL' : "'$jam_masuk'";
      $jam_pulang_sql = ($jam_pulang === '' || $jam_pulang === null) ? 'NULL' : "'$jam_pulang'";
      $status_masuk_sql = ($status_masuk === '' || $status_masuk === null) ? 'NULL' : "'$status_masuk'";
      $status_pulang_sql = ($status_pulang === '' || $status_pulang === null) ? 'NULL' : "'$status_pulang'";
      $kehadiran_sql = ($kehadiran === '' || $kehadiran === null) ? 'NULL' : "'$kehadiran'";

      $query = "UPDATE absensi 
        SET tanggal = '$tanggal',
            jam_masuk = $jam_masuk_sql,
            status_masuk = $status_masuk_sql,
            jam_pulang = $jam_pulang_sql,
            status_pulang = $status_pulang_sql,
            kehadiran = $kehadiran_sql,
            updated_at = NOW()
        WHERE id = '$id'";
    ...
    case 'delete':
      ...
      $query_foto = $connection->query("SELECT foto_masuk, foto_pulang FROM absensi WHERE id='$id'");
      ...
      $delete = $connection->query("DELETE FROM absensi WHERE id='$id'");
      if ($delete) {
    ...
    case 'approve_manual':
      ...
      $q = $connection->query("UPDATE absensi SET approval_status='approved', approved_by='$admin_id', approved_at=NOW(), updated_at=NOW() WHERE id='$id' AND approval_status='pending'");
      if ($q) {
    ...
    case 'reject_manual':
      ...
      $id = intval($_POST['id']);
      $admin_id = intval($_SESSION['admin_id'] ?? $ptk_id ?? 0);
      $q = $connection->query("UPDATE absensi SET approval_status='rejected', approved_by='$admin_id', approved_at=NOW(), updated_at=NOW() WHERE id='$id' AND approval_status='pending'");
      if ($q) {
