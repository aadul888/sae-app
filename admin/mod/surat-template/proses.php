...
switch ($action) {
  ...
  case 'delete':
    ...
    $id = (int)($_POST['id'] ?? 0);
    if ($id <= 0) { echo 'ID tidak valid.'; break; }
    echo $connection->query("DELETE FROM surat_template WHERE id=$id") ? 'success' : 'Gagal: ' . $connection->error;
    break;
  ...
  case 'scan_tags':
    ...
    try {
      ...
      $escaped = $connection->real_escape_string($tags_str);
      $connection->query("UPDATE surat_template SET variabel_tag='$escaped' WHERE id=$template_id");
