...
switch ($action) {
  ...
  case 'delete':
    ...
    $id = (int)($_POST['id'] ?? 0);
    if ($id <= 0) { echo 'ID tidak valid.'; break; }
    $stmt_dsi = $connection->prepare("DELETE FROM surat_index WHERE id=?");
    $stmt_dsi->bind_param('i', $id);
    echo $stmt_dsi->execute() ? 'success' : 'Gagal: ' . $connection->error;
    $stmt_dsi->close();
    break;
  ...
  case 'import_excel':
    ...
    try {
      ...
      for ($i = 1; $i < count($rows); $i++) {
        ...
        if ($iv === '') continue;
        $pv = trim($rows[$i][$cp] ?? '');
        $kv = $ck !== false ? trim($rows[$i][$ck] ?? '') : '';
        $jv = $cj !== false ? trim($rows[$i][$cj] ?? '') : 'Surat Keluar';
        if ($jv === '') $jv = 'Surat Keluar';
        // get next id
        $res_nx = $connection->query("SELECT COALESCE(MAX(id),0)+1 AS n FROM surat_index");
        $nx = (int)$res_nx->fetch_row()[0];
        $ct = sprintf('%04d/%s-SMKN1PGL', $nx, $iv);
        $stmt_imp = $connection->prepare("INSERT INTO surat_index (indeks,perihal,kategori,jenis_surat,contoh_nomor) VALUES (?,?,?,?,?)");
        $stmt_imp->bind_param('sssss', $iv, $pv, $kv, $jv, $ct);
        if ($stmt_imp->execute()) $ok++;
        $stmt_imp->close();
      }
  ...
  case 'kategori_delete':
    ...
    $stmt_dsk = $connection->prepare("DELETE FROM surat_kategori WHERE id=?");
    $stmt_dsk->bind_param('i', $id);
    echo $stmt_dsk->execute() ? json_encode(['status' => 'success']) : json_encode(['status' => 'error', 'message' => $connection->error]);
    $stmt_dsk->close();
    break;
  ...
  case 'jenis_delete':
    ...
    $id = (int)($_POST['id'] ?? 0);
    if ($id <= 0) { echo json_encode(['status' => 'error', 'message' => 'ID tidak valid.']); break; }
    $stmt_dsj = $connection->prepare("DELETE FROM surat_jenis WHERE id=?");
    $stmt_dsj->bind_param('i', $id);
    echo $stmt_dsj->execute() ? json_encode(['status' => 'success']) : json_encode(['status' => 'error', 'message' => $connection->error]);
    $stmt_dsj->close();
    break;
