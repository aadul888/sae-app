    ...
    if ($new_status === 'Ditolak' && $catatan === '') {
        ...
        if ($ok) {
                    ...
                    if ($is_security_actor && in_array($konfirmasi_req, ['keluar', 'kembali', 'pulang'], true)) {
                        ...
                        if ($konfirmasi_req === 'keluar') {
                            try {
                                ...
                                $return_link2 = rtrim($protocol . '://' . $host . $app_root, '/') . '/admin/mod/e-izin/approve.php?t=' . $newTok2 . '&role=security&action=return';
                                $qr_img2 = rtrim($protocol . '://' . $host . $app_root, '/') . '/dashboard/mod/e-izin/qrcode.php?id=' . intval($id) . '&role=security&action=return';
            ...
            if ($summary_ready && check_session_permission($connection, $level_id, $tugas_arr, 'admin')) {
                ...
                if (trim($app_root, '/') === 'admin') $root_prefix = '';
