...
switch (@$_GET['action']) {
  ...
  case 'hapus':
    $id = anti_injection($_POST['id']);
    $check = $connection->query("SELECT id FROM e_izin WHERE id='$id'");
    if ($check->num_rows > 0) {
      $delete = $connection->query("DELETE FROM e_izin WHERE id='$id'");
  ...
  case 'edit_catatan':
    ...
    $check = $connection->query("SELECT id FROM e_izin WHERE id='$id' AND status_izin='Ditolak'");
    if ($check->num_rows > 0) {
      $update = $connection->query("UPDATE e_izin SET alasan_penolakan='$catatan' WHERE id='$id'");
