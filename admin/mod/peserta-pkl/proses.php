...
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])) {
  ...
  $cfg_file = __DIR__ . '/../sync/sync_config.json';
  ...
  function pkl_load_config($cfg_file)
  {
    ...
    if (is_file($cfg_file)) {
      ...
      if (is_array($tmp) && isset($tmp['pkl']) && is_array($tmp['pkl'])) {
        $cfg = array_merge($cfg, $tmp['pkl']);
      } elseif (is_array($tmp)) {
        // Fallback: if it's old format (flat keys), merge directly
