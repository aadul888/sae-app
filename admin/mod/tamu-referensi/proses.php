...
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])) {
  ...
  switch ($action) {
    ...
    case 'hapus_instansi':
      ...
      $connection->query("DELETE FROM tamu_instansi WHERE id=$id");
      echo $connection->affected_rows > 0 ? 'Instansi berhasil dihapus' : 'Gagal menghapus';
      break;
    ...
    case 'hapus_tujuan':
      ...
      $id = intval($_POST['id'] ?? 0);
      if (!$id) { echo 'ID tidak valid'; exit; }
      $connection->query("DELETE FROM tamu_tujuan WHERE id=$id");
      echo $connection->affected_rows > 0 ? 'Tujuan berhasil dihapus' : 'Gagal menghapus';
      break;
