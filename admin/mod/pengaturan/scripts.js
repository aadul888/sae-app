...
function backupFiles() {
  ...
  swal({
    ...
    text:
      "Sistem akan scan folder " +
      selectedFolder +
      " dan membuat file ZIP. Proses akan otomatis mengunduh saat selesai.",
    icon: "info",
    buttons: ["Batal", "Mulai"],
    dangerMode: false,
  }).then((willBackup) => {
  ...
  $.ajax({
    ...
    success: function (scanRes) {
      if (!scanRes || scanRes.status !== "success") {
        swal(
          "Gagal",
          (scanRes && scanRes.message) || "Gagal memindai aset backup.",
          "error",
        );
        return;
      }
      ...
      $.ajax({
        ...
        success: function (createRes) {
          if (!createRes || createRes.status !== "success") {
            swal(
              "Gagal",
              (createRes && createRes.message) || "Gagal membuat ZIP backup.",
              "error",
            );
            return;
          }
          ...
          swal(
            "Berhasil",
            "Unduhan backup folder " +
              (data.label || folder) +
              " dimulai tanpa menyimpan file temp di server.",
            "success",
          );
        },
        error: function (xhr) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (trimmed === "success") {
            ...
            swal({
              title: "Gagal",
              text: trimmed || "Gagal menghapus modul.",
              icon: "error",
              timer: 2500,
            });
          }
        ...
        error: function (xhr) {
          swal({
            title: "Error",
            text: xhr.statusText || "Terjadi kesalahan.",
            icon: "error",
            timer: 2500,
          });
        },
...
/** Hapus Semua Data Sync Dapodik */
function clearSyncData() {
  swal({
    title: "Hapus Semua Data Sync?",
    text: "Seluruh data sync Dapodik (7 tabel + log) akan dihapus permanen. Tindakan ini tidak dapat dibatalkan!",
    icon: "warning",
    ...
    swal({
      title: "Menghapus...",
      text: "Menghapus seluruh data sync Dapodik",
      icon: "info",
      buttons: false,
      closeOnClickOutside: false,
      closeOnEsc: false,
    ...
    $.ajax({
      url: "mod/pengaturan/proses.php?action=clear-sync-tables",
      type: "POST",
      dataType: "json",
      success: function (res) {
        if (res.status === "success") {
          swal({
            ...
            text:
              res.message ||
              "Semua data sync berhasil dihapus. Mengarahkan ke halaman utama...",
            icon: "success",
            timer: 2000,
          }).then(() => {
          ...
          swal({
            title: "Gagal",
            text: res.message || "Gagal menghapus data sync.",
            icon: "error",
            timer: 2500,
          });
        }
      ...
      error: function (xhr) {
        swal({
          title: "Error",
          text: xhr.statusText || "Terjadi kesalahan saat menghapus data sync.",
          icon: "error",
          timer: 2500,
        });
      },
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (trimmed === "success") {
        ...
        swal({
          title: "Oops!",
          text: trimmed || "Terjadi kesalahan saat upload.",
          icon: "error",
          timer: 3500,
        });
      }
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (trimmed === "success") {
        swal(
          "Berhasil!",
          "Pengaturan maintenance berhasil disimpan",
          "success",
        );
        loadSetting(8);
      } else {
        swal(
          "Gagal!",
          trimmed || "Terjadi kesalahan saat menyimpan pengaturan maintenance",
          "error",
        );
      }
    },
