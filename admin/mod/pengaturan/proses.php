...
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])) {
  ...
  switch (@$_GET['action']) {
    ...
    case 'setting-web':
      ...
      // Alamat: 1 field textarea (site_address) sudah cukup
      // Field alamat detail lama tidak lagi dikirim dari form — set kosong
      $alamat_jalan    = '';
      $rt              = '';
      $rw              = '';
      $dusun           = '';
      ...
      if (empty($error)) {
        $update = "UPDATE setting SET site_name='$site_name',
                ...
                provinsi='$provinsi',
                kode_pos='$kode_pos' WHERE site_id=$real_site_id";
        if ($connection->query($update) === false) {
          echo 'Pengaturan tidak dapat disimpan, coba ulangi beberapa saat lagi.!';
          die($connection->error . __LINE__);
        } else {
    ...
    case 'tapel-save':
      ...
      if ($aktif == 'Y') {
        ...
        $connection->query("UPDATE tahun_pelajaran SET aktif='N' WHERE 1");  // OK: no user vars
      }
      if ($id > 0) {
        // Edit tahun pelajaran
        $stmt_tp = $connection->prepare("UPDATE tahun_pelajaran SET tahun=?, semester=?, aktif=? WHERE id=?");
        $stmt_tp->bind_param('sssi', $tahun, $semester, $aktif, $id);
        if (!$stmt_tp->execute()) {
        ...
        $stmt_tp_ins = $connection->prepare("INSERT INTO tahun_pelajaran (tahun, semester, aktif) VALUES (?, ?, ?)");
        $stmt_tp_ins->bind_param('sss', $tahun, $semester, $aktif);
        if (!$stmt_tp_ins->execute()) {
    ...
    case 'system-status':
      ...
      if ($system_status === 'closed') {
        $connection->query("UPDATE tahun_pelajaran SET aktif='N'");  // OK: constant
        echo 'success';
        break;
      }
      ...
      $stmt_cek = $connection->prepare("SELECT id FROM tahun_pelajaran WHERE id=? LIMIT 1");
      $stmt_cek->bind_param('i', $tapel_id);
      $stmt_cek->execute();
      ...
      $connection->query("UPDATE tahun_pelajaran SET aktif='N'");  // OK: constant
      $stmt_st = $connection->prepare("UPDATE tahun_pelajaran SET aktif='Y' WHERE id=?");
      $stmt_st->bind_param('i', $tapel_id);
      $stmt_st->execute();
      $stmt_st->close();
    ...
    case 'maintenance-setting':
      ...
      $stmt_ms = $connection->prepare("UPDATE setting SET maintenance_status=? WHERE site_id=?");
      $stmt_ms->bind_param('si', $maintenance_status, $real_site_id);
      if (!$stmt_ms->execute()) {
    ...
    /** Hapus semua tabel sync dari Dapodik */
    case 'clear-sync-tables':
      ...
      echo json_encode(['status' => 'success', 'message' => 'Semua data sync Dapodik berhasil dihapus.']);
      break;
  }
}
