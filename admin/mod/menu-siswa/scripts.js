(function ($) {
  $(function () {
    ...
    $(document).on("click", ".btn-add", function () {
      ...
      $.get(
        "./mod/menu-siswa/datatable.php?get_max_position=1",
        function (resp) {
          try {
            var data = JSON.parse(resp);
            if (data.max_position) {
              $("#menuSiswaForm [name=position]").val(
                parseInt(data.max_position) + 10,
              );
            }
          } catch (e) {
    ...
    $(document).on("submit", "#menuSiswaForm", function (e) {
      ...
      if (!slugPattern.test(slug)) {
        alert(
          "Slug hanya boleh mengandung huruf, angka, tanda minus (-) dan underscore (_)",
        );
        return;
      }
