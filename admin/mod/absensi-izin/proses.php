...
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])) {
  ...
  switch (@$_GET['action']) {
    ...
    case 'setujui':
      ...
      $id = $_POST['id'];
      $stmt = $connection->prepare("SELECT id FROM izin WHERE id=? AND status_izin='Menunggu'");
      $stmt->bind_param('s', $id);
      $stmt->execute();
      $stmt->store_result();
      if ($stmt->num_rows > 0) {
        $stmt->close();
        $stmt2 = $connection->prepare("UPDATE izin SET status_izin='Disetujui', alasan_penolakan='' WHERE id=?");
    ...
    case 'tolak':
      ...
      if ($stmt->num_rows > 0) {
        $stmt->close();
        $stmt2 = $connection->prepare("UPDATE izin SET status_izin='Ditolak', alasan_penolakan=? WHERE id=?");
        $stmt2->bind_param('ss', $alasan, $id);
        $stmt2->execute();
        $stmt2->close();
    ...
    case 'detail':
      check_access('lihat');
      $id = $_POST['id'];
      $stmt = $connection->prepare("SELECT izin.*, user.nama_lengkap, user.nisn, kelas.nama_kelas FROM izin
    ...
    case 'hapus':
      check_access('hapus');
      $id = $_POST['id'];
      $stmt = $connection->prepare("SELECT id FROM izin WHERE id=?");
      $stmt->bind_param('s', $id);
      $stmt->execute();
      $stmt->store_result();
      if ($stmt->num_rows > 0) {
        $stmt->close();
        $stmt2 = $connection->prepare("DELETE FROM izin WHERE id=?");
        $stmt2->bind_param('s', $id);
    ...
    case 'edit_catatan':
      ...
      $stmt = $connection->prepare("SELECT id FROM izin WHERE id=? AND status_izin='Ditolak'");
      $stmt->bind_param('s', $id);
      $stmt->execute();
      $stmt->store_result();
      if ($stmt->num_rows > 0) {
        $stmt->close();
        $stmt2 = $connection->prepare("UPDATE izin SET alasan_penolakan=? WHERE id=?");
