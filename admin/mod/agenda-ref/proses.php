...
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])) {
  ...
  switch ($action) {
    ...
    case 'hapus':
      ...
      $stmt_h = $connection->prepare("DELETE FROM agenda_mapel WHERE mapel_id=?");
      $stmt_h->bind_param('i', $id);
      echo $stmt_h->execute() && $stmt_h->affected_rows > 0 ? 'Berhasil dihapus' : 'Gagal menghapus';
      $stmt_h->close();
      break;
