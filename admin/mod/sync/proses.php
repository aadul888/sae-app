...
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY']) && !$allow_public_bootstrap) {
  ...
  function get_sync_config_path()
  {
    // Use writable temp dir — avoids permission issues on shared hosting where
    // the module directory is read-only. Falls back to __DIR__ if temp not writable.
    $tmp = @sys_get_temp_dir();
    if ($tmp !== false && is_writable($tmp)) {
      return $tmp . '/sae_sync_config.json';
    }
    return __DIR__ . '/sync_config.json';
  }
  ...
  function load_sync_config()
  {
    ...
    if (!file_exists($path)) {
      // Migrate from old individual files if they exist
      $old_files = [
        'dapodik' => __DIR__ . '/dapodik_sync_config.json',
        'connection' => __DIR__ . '/connection_status.json',
        'progress' => __DIR__ . '/current_progress.json',
        'pkl' => __DIR__ . '/pkl_sync_config.json',
      ];
      $has_old = false;
      foreach ($old_files as $section => $old_path) {
    ...
    foreach ($defaults as $section => $default_values) {
      if (!isset($decoded[$section]) || !is_array($decoded[$section])) {
        $decoded[$section] = $default_values;
      } else {
        $decoded[$section] = array_merge($default_values, $decoded[$section]);
      }
    }
  ...
  function save_sync_config($data)
  {
    @file_put_contents(
  ...
  function push_sync_progress($endpoint, $status, $message, $context = [])
  {
    ...
    // Tulis ke consolidated config
    $full_cfg = load_sync_config();
    $full_cfg['progress'] = [
      'updated_at' => $entry['timestamp'],
      'current' => $entry,
      'items' => $items
    ];
    save_sync_config($full_cfg);
  }
  ...
  function sync_default_password_hash()
  {
    static $hash = null;
    if ($hash === null) {
  ...
  function sync_resolve_user_id_from_nisn($nisn)
  {
    $digits = preg_replace('/\D+/', '', (string) $nisn);
    if ($digits === '') {
      return null;
    }
    ...
    if (strlen($normalized) === 10 && strcmp($normalized, '2147483647') > 0) {
      return null;
    }
  ...
  {
    $cfg = load_sync_config();
    $cfg['connection'] = array_merge($cfg['connection'], [
      'connection_status' => $status,
      'updated_at' => date('c'),
      'message' => $message
    ]);
    if (!empty($details)) {
      $cfg['connection']['details'] = $details;
    }
    save_sync_config($cfg);
  ...
  switch (@$_GET['action']) {
    ...
    case 'set-connection-status':
      ...
      $cfg = load_sync_config();
      if ($type === 'api') {
        $cfg['connection']['api_connection_status'] = $new_status;
        $cfg['connection']['api_updated_at'] = date('c');
      } else {
        $cfg['connection']['connection_status'] = $new_status;
        $cfg['connection']['updated_at'] = date('c');
      }
      save_sync_config($cfg);

      header('Content-Type: application/json');
      echo json_encode(['status' => 'success', 'data' => $cfg['connection']]);
      exit;
    ...
    case 'get-status':
      ...
      // Ambil status dari consolidated config
      $cfg = load_sync_config();
      $file_data = $cfg['connection'];
    ...
    case 'getPengguna':
      ...
      */
      ...
      $processed = 0; $updated = 0; $inserted = 0; $skipped = 0; $failed = 0; $deactivated = 0; $deleted = 0;
      $failed_msgs = [];
      $default_password_hash = sync_default_password_hash();
      ...
      while ($p = $result->fetch_assoc()) {
        ...
        $password       = !empty($p['password']) ? $p['password'] : $default_password_hash;
        $phone          = $p['no_hp'] ?? '';
        $ptk_id         = $p['ptk_id'] ?? '';
        $email          = $username;
        ...
        if (!$cek) {
          throw new RuntimeException('Prepare cek admin gagal: ' . $connection->error);
        }
        ...
        if ($row) {
          // UPDATE â€” jaga avatar, gelar, tugas_tambahan yang diisi manual
          ...
          if (!$upd) {
            throw new RuntimeException('Prepare update admin gagal: ' . $connection->error);
          }
          ...
          if (!$ins) {
            throw new RuntimeException('Prepare insert admin gagal: ' . $connection->error);
          }
      ...
      $cleanup_sql = "
        ...
        LEFT JOIN sync_pengguna sp
          ON TRIM(COALESCE(sp.pengguna_id, '')) = TRIM(COALESCE(a.pengguna_id, ''))
          AND sp.username IS NOT NULL
          AND TRIM(sp.username) <> ''
          AND TRIM(COALESCE(sp.peran_id_str, '')) <> 'Peserta Didik'
        WHERE sp.pengguna_id IS NULL
          AND a.admin_id != " . $current_admin_id . "
          AND (
            TRIM(COALESCE(a.pengguna_id, '')) <> ''
            OR COALESCE(a.username, '') = 'admin@sae.id'
            OR COALESCE(a.sync_status, 'manual') = 'manual'
          )
      ";
      if ($connection->query($cleanup_sql)) {
        $deleted = (int) $connection->affected_rows;
      } else {
        $failed++;
        $failed_msgs[] = 'Hapus pengguna lama gagal: ' . $connection->error;
      }

      $msg = "Processed: $processed, Updated: $updated, Inserted: $inserted, Skipped: $skipped, Deactivated: $deactivated, Deleted: $deleted, Failed: $failed";
      if ($failed > 0) $msg .= "\nDetails: " . implode('; ', $failed_msgs);
      ...
      exit;
        ...
        $error_msg = 'Sinkronisasi Pengguna dihentikan: ' . $e->getMessage() . ' (baris ke-' . $processed . ' dari ' . ($total_rows ?? 0) . ')';
        push_sync_progress('getPengguna', 'failed', $error_msg, [
          'scope' => 'table',
          'processed' => $processed,
          'total' => $total_rows ?? 0,
          'updated' => $updated,
        ...
        save_sync_log('getPengguna', 'failed', $processed, $error_msg);
    ...
    case 'getGtk':
      ...
      */
      ...
      while ($gtk = $result->fetch_assoc()) {
        ...
        $password    = $pengguna
          ? (!empty($pengguna['password']) ? $pengguna['password'] : $default_password_hash)
        ...
        if ($row) {
          ...
          if (!$ins) {
            $failed++;
            $failed_msgs[] = 'Prepare insert admin (GTK) gagal: ' . $connection->error;
            continue;
          }
          // 24 params, gunakan string untuk menjaga fleksibilitas casting MySQL
          $ins->bind_param(str_repeat('s', 24),
      ...
      */
      ...
      try {
        ...
        while ($siswa_data = $result->fetch_assoc()) {
          ...
          try {
            $resolved_user_id = sync_resolve_user_id_from_nisn($siswa_data['nisn'] ?? '');
            if ($resolved_user_id === null) {
              $skipped++;
              $failed_msgs[] = 'Siswa ' . (($siswa_data['nama'] ?? '') !== '' ? $siswa_data['nama'] : ('Baris ' . $processed)) . ' dilewati karena NISN tidak valid untuk user_id.';
              continue;
            }
        ...
        if ($rbid !== '' && $kelas_stmt) {
          ...
          if ($kelas_res && $kelas_res->num_rows > 0) {
            ...
            if ($check_result->num_rows > 0) {
              // Update data yang sudah ada â€” hanya update bila ada perbedaan
              ...
              if ((int) ($user['user_id'] ?? 0) !== $resolved_user_id) {
                $id_conflict = $connection->prepare("SELECT user_id FROM user WHERE user_id = ? AND user_id != ? LIMIT 1");
                if ($id_conflict) {
                  $current_user_id = (int) ($user['user_id'] ?? 0);
                  $id_conflict->bind_param('ii', $resolved_user_id, $current_user_id);
                  $id_conflict->execute();
                  $id_conflict_res = $id_conflict->get_result();
                  if (!$id_conflict_res || $id_conflict_res->num_rows === 0) {
                    $update_fields[] = "user_id=?";
                    $update_values[] = $resolved_user_id;
                    $diff_found = true;
    ...
    case 'generate-api-key':
      ...
      // Simpan API key ke tabel setting (kolom api_key).
      // Pendekatan DB menghindari error permission file pada shared hosting.
      $success = false;
      $write_error = '';
      if ($connection) {
        // UPSERT via UPDATE-then-INSERT (setting.site_id belum punya UNIQUE/PK).
        $stmt = $connection->prepare("UPDATE setting SET api_key = ? WHERE site_id = 1");
        if ($stmt) {
          $stmt->bind_param('s', $new_api_key);
          $success = $stmt->execute();
          if (!$success) $write_error = $stmt->error ?: $connection->error;
          $stmt->close();
        } else {
          $write_error = 'prepare fail: ' . $connection->error;
        }
        if ($success && $connection->affected_rows === 0) {
          $stmt = $connection->prepare("INSERT INTO setting (site_id, api_key) VALUES (1, ?)");
          if ($stmt) {
            $stmt->bind_param('s', $new_api_key);
            $success = $stmt->execute();
            if (!$success) $write_error = $stmt->error ?: $connection->error;
            ...
            $write_error = 'insert prepare fail: ' . $connection->error;
          }
        }
      } else {
        $write_error = 'no connection';
      }
      if ($success) {
        ...
        echo json_encode([
          ...
          'api_key' => $new_api_key,
          'security_note' => 'Key tersimpan di tabel setting (kolom api_key)'
        ]);
      } else {
        $diagnostic = $write_error !== '' ? ' Detail: ' . substr($write_error, 0, 180) : '';
        echo json_encode([
          'status' => 'error',
          'message' => 'Gagal menyimpan API Key ke database.' . $diagnostic,
          'api_key' => $new_api_key,
