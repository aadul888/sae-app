...
switch ($action) {
  ...
  case 'datatable':
    ...
    if ($q) {
      ...
      while ($sk = $q->fetch_assoc()) {
        ...
        // PDF link â€” prioritaskan drive_view_link
        $pdfLink = '';
  ...
  case 'kirim':
    ...
    $stmt_sk_stat = $connection->prepare("UPDATE surat_keluar SET status='Terkirim' WHERE id=?");
    $stmt_sk_stat->bind_param('i', $id);
    $stmt_sk_stat->execute();
    $stmt_sk_stat->close();
  ...
  case 'delete':
    ...
    $stmt_dsk = $connection->prepare("DELETE FROM surat_keluar WHERE id=?");
    $stmt_dsk->bind_param('i', $id);
    echo $stmt_dsk->execute() ? 'success' : 'Gagal.';
    $stmt_dsk->close();
    break;
  ...
  // ====== LOAD TEMPLATE TAGS â€” kirim fields dari variabel_tag ======
  case 'load_template_tags':
    ...
    // Parse {{tag}} from variabel_tag string â€” urut sesuai kemunculan di template
  ...
  // ====== GENERATE SURAT â€” copy template Google Docs, replace text via Docs API ======
  case 'generate_pdf':
    header('Content-Type: application/json');
    sk_check('modifikasi');
    ...
    // Ambil setting kredensial â€” dukung OAuth 2.0 dan Service Account
    ...
    try {
      ...
      // =================================================================
      // STRATEGI BARU: Copy template â†’ Replace text via Docs API â†’ Selesai
      ...
      // 1. Copy template â€” otomatis konversi ke Google Docs jika file Office
      ...
      // 2. Siapkan replacement â€” ganti {{tags}} dengan nilai dari form
      $replacements = [];
      ...
      if ($surat_id_editing > 0) {
        ...
        $stmt_edit = $connection->prepare("UPDATE surat_keluar SET template_path=?, template_fields_json=?, drive_file_id=?, drive_view_link=?, updated_at=NOW() WHERE id=?");
    $stmt_edit->bind_param('ssssi', $tpath, $fields_json, $efid, $elink, $surat_id_editing);
    $stmt_edit->execute();
    $stmt_edit->close();
      }
