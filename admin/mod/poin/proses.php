    ...
    case 'hapus_pelanggaran':
        ...
        if ($stmt->execute()) {
            ...
            if (!empty($files['bukti_foto'])) {
                $foto_path = $_SERVER['DOCUMENT_ROOT'] . '/content/pelanggaran/foto/' . $files['bukti_foto'];
                if (file_exists($foto_path)) @unlink($foto_path);
            }
            // Delete video file
            if (!empty($files['bukti_video'])) {
                $video_path = $_SERVER['DOCUMENT_ROOT'] . '/content/pelanggaran/video/' . $files['bukti_video'];
                if (file_exists($video_path)) @unlink($video_path);
            }
            // Legacy: satu kolom bukti
            if (!empty($files['bukti'])) {
                $legacy = $files['bukti'];
                $path1 = $_SERVER['DOCUMENT_ROOT'] . '/content/pelanggaran/foto/' . $legacy;
                $path2 = $_SERVER['DOCUMENT_ROOT'] . '/content/pelanggaran/video/' . $legacy;
