<?php session_start();
// Buffer output — strip any notice/warning so JSON response stays valid
ob_start();
if (!isset($_COOKIE['ADMIN_KEY']) && !isset($_COOKIE['KEY'])) {
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode(array("draw" => 0, "recordsTotal" => 0, "recordsFiltered" => 0, "data" => array(), "error" => "unauthorized"));
    exit;
} else {
    ...
    // Hak akses modul Admin (modul_id=4)
    $modul_id = 4;
    ...
    $gaSql['link'] =  new mysqli($gaSql['server'], $gaSql['user'], $gaSql['password'], $gaSql['db'], defined('DB_PORT') ? (int) DB_PORT : ini_get('mysqli.default_port'));
