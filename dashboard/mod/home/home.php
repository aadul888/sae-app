<?php
$show_modal = false; // modal pembaharuan nonaktif — riwayat via modul Pembaruan admin
...
<?php endif; ?>
<?php
// --- Info Popup untuk Dashboard Siswa ---
$info_siswa = [];
...
$cookie_hide_info = $_COOKIE['hide_info_siswa'] ?? '';
$show_info_siswa = count($info_siswa) > 0;
if ($show_info_siswa && $cookie_hide_info !== '') {
    $show_info_siswa = false;
}
?>

<?php if ($show_info_siswa): ?>
  <div class="modal fade" id="modalInfoSiswa" tabindex="-1">
    <div class="modal-dialog modal-md" role="document">
      <div class="modal-content">
        <div class="modal-header bg-gradient-primary text-white">
          <h6 class="modal-title text-white"><i class="fas fa-info-circle me-2"></i>Informasi</h6>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body" style="max-height:350px;overflow-y:auto;">
          <?php foreach ($info_siswa as $row): ?>
            <div class="mb-3 pb-2 border-bottom border-light">
              <h6 class="mb-1">
                <?php if (!empty($row['kategori'])): ?><span class="badge bg-info me-2"><?php echo ucfirst(htmlspecialchars($row['kategori'])); ?></span><?php endif; ?>
              ...
              <div class="small"><?php echo $row['konten']; ?></div>
              <small class="text-muted mt-1 d-block"><i class="fas fa-clock me-1"></i><?php echo date('d/m/Y H:i', strtotime($row['created_at'])); ?></small>
            </div>
          <?php endforeach; ?>
        </div>
        <div class="modal-footer border-0">
          <button type="button" id="hideInfoSiswa" class="btn btn-outline-secondary btn-sm">Jangan Tampilkan Lagi</button>
  ...
  document.addEventListener("DOMContentLoaded", function() {
    var m = new bootstrap.Modal(document.getElementById('modalInfoSiswa'));
    m.show();
  });
  document.getElementById('hideInfoSiswa')?.addEventListener('click', function() {
    document.cookie = "hide_info_siswa=1; path=/; max-age=" + (60*60*24*30);
    bootstrap.Modal.getInstance(document.getElementById('modalInfoSiswa'))?.hide();
  });
  </script>
<?php endif; ?>
<?php
