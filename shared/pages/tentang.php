...
// Ambil versi dari database (app_version) jika tersedia
$sae_about_version = 'v5.0';
$sae_about_commit = '';
if (isset($connection) && $connection) {
    $qv = $connection->query("SELECT app_version, last_deploy_commit FROM setting WHERE site_id=1 LIMIT 1");
    if ($qv && $qv->num_rows) {
        $vr = $qv->fetch_assoc();
        if (!empty($vr['app_version'])) $sae_about_version = 'v' . $vr['app_version'];
        if (!empty($vr['last_deploy_commit'])) $sae_about_commit = substr($vr['last_deploy_commit'], 0, 7);
    }
}
if (defined('SAE_VERSION') && empty($sae_about_version)) $sae_about_version = (string)SAE_VERSION;
