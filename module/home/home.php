...
if ($is_standalone) {
    ...
    <div class="module-home-container">
                    ...
                    <div class="sae-hero-inner">
                        ...
                        <div class="sae-hero-right">
                            ...
                            if (isset($connection) && $connection) {
                                // Detect available columns to build a resilient query
                                $info_cols = ['judul', 'konten'];
                                $info_where = "aktif=1";
                                $info_order = "id DESC";
                                $col_check = $connection->query("SHOW COLUMNS FROM info");
                                if ($col_check) {
                                    $existing = [];
                                    while ($c = $col_check->fetch_assoc()) $existing[] = $c['Field'];
                                    if (in_array('kategori', $existing)) $info_cols[] = 'kategori';
                                    if (in_array('tgl_mulai', $existing)) $info_where .= " AND (tgl_mulai IS NULL OR tgl_mulai <= CURDATE())";
                                    if (in_array('tgl_selesai', $existing)) $info_where .= " AND (tgl_selesai IS NULL OR tgl_selesai >= CURDATE())";
                                    if (in_array('urutan', $existing)) $info_order = "urutan DESC, id DESC";
                                }
                                $q_info = @$connection->query("SELECT " . implode(',', $info_cols) . " FROM info WHERE {$info_where} ORDER BY {$info_order} LIMIT 5");
                                if ($q_info && $q_info->num_rows > 0) {
                                    $info_items = [];
                                    while ($row_info = $q_info->fetch_assoc()) {
                                        $prefix = !empty($row_info['kategori']) ? '[' . ucfirst($row_info['kategori']) . '] ' : '';
                                        $info_items[] = $prefix . $row_info['judul'] . (!empty($row_info['konten']) ? ': ' . $row_info['konten'] : '');
                                    }
                                    $info_running = implode('  ●  ', $info_items);
                                }
                            }
                            ?>
                            <?php if (!empty($info_running)): ?>
                            <div class="sae-ticker" aria-label="Informasi Sekolah">
                                <div class="sae-ticker-label">
                                    <i class="fas fa-bullhorn"></i>
                                    <span>Info</span>
                                </div>
                                <div class="sae-ticker-track">
                                    <div class="sae-ticker-content">
                                        <span><?php echo $info_running; ?></span>
                                        <span><?php echo $info_running; ?></span>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="module-footer">
            ...
            &nbsp;|&nbsp; Developed by <a href="https://www.instagram.com/aadul888/" target="_blank" rel="noopener noreferrer">aadul888</a>
        </div>
        <?php endif; ?>
