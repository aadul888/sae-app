...
switch ($action) {
    case 'scan_rfid':
        // Rate limiting: max 30 scan per IP per menit (cegah brute-force/spam)
        if (!rate_limit_check($connection, 'rfid_scan', 30, 60)) {
            echo 'Terlalu banyak percobaan. Silakan coba lagi nanti.';
            exit;
        }
        ...
        if (!empty($foto_capture) && strpos($foto_capture, 'data:image/jpeg;base64,') === 0) {
            ...
            if ($img_data !== false) {
                $nisn = '';
                // Ambil nisn dari user jika sudah ada (prepared statement)
                $stmt_nisn = $connection->prepare("SELECT nisn FROM user WHERE rfid = ? LIMIT 1");
                if ($stmt_nisn) {
                    $stmt_nisn->bind_param('s', $rfid);
                    $stmt_nisn->execute();
        ...
        // Ambil data user beserta nama kelas (prepared statement — cegah SQL injection)
        $stmt_user = $connection->prepare("SELECT u.nisn, u.nama_lengkap, u.status, u.user_id, k.nama_kelas, u.avatar
                FROM user u
                LEFT JOIN kelas k ON u.kelas = k.kelas_id
                WHERE u.rfid = ? LIMIT 1");
        if (!$stmt_user) {
            echo 'Terjadi kesalahan sistem.';
            exit;
        }
        $stmt_user->bind_param('s', $rfid);
        $stmt_user->execute();
